...
export const LOCATORS = {
  ...
  staticIpsToggleButton:
    'input[name="accessSettings.staticIpAllocationEnabled"]',
  ...
  dynamicIpInputField:
    '[data-testid="entitytextinputfield-root-vpnRange-input"] input',
  saveButton: '[data-testid="editor-submit-btn"]',
  popUpMessage: "#notistack-snackbar",
  staticIpInputField:
    '[data-testid="entitytextinputfield-root-accessSettings.staticIpRange-input"] input',
  sdpUserInput:
    '[data-testid="entityautocompletefieldprops-inputuser"] [placeholder="Search or select SDP User"]',
  listBox: ".MuiV5-Autocomplete-listbox li",
  sdpIPInput:
    '[data-testid="entitytextinputfield-root-ip-input"] [placeholder="Insert IP from defined range"]',
  ...
  tableTr: "table tr",
  overrideButton:
    '[data-testid="catodialog-actions"] > .MuiButton-contained > .MuiButton-label',
...
export const TEXT_CONTENT = {
  ...
  ipRange: "IP Range",
  staticIp: "Static IP",
  allocateApreDefinedIp:
    "Allocate a pre-defined static IP address to SDP users, instead of from the dynamic IP range",
  enableStaticIPs: "Enable Static IPs",
  warningMessage:
    "When disabled, all IPs are only allocated from the Dynamic IP range",
...
export function validatePageContent() {
  ...
  cy.get(LOCATORS.pageTitle).should(
    "have.text",
    TEXT_CONTENT.iPAllocationPolicy
  );
  cy.get(LOCATORS.dynamicIp).should("contain", TEXT_CONTENT.dynamicIp);
  cy.get(LOCATORS.dynamicIpText1).should(
    "contain",
    TEXT_CONTENT.defineTheDefaultIp
  );
  ...
  cy.get(LOCATORS.staticIpText1).should(
    "contain",
    TEXT_CONTENT.allocateApreDefinedIp
  );
  cy.get(LOCATORS.enableStaticIps).should(
    "contain.text",
    TEXT_CONTENT.enableStaticIPs
  );
  ...
  cy.get(LOCATORS.allocateIpPerSDPUser).should(
    "contain.text",
    TEXT_CONTENT.allocateIpPerSDPUser
  );
...
export function chooseFromDropDown(
  dropdownSelector,
  optionText,
  searchSelector
) {
