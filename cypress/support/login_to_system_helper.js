...
export const LOCATORS = {
  ...
  userAndGroupDropdownMenu1:
    '[data-testid="section"]:nth-child(3) .MuiV5-Autocomplete-inputRoot input',
  userAndGroupDropdownMenu2:
    '[data-testid="section"]:nth-child(3) [placeholder*="Search or select SDP User"]',
  userAndGroupListBox:
  ...
  platformDropDownMenu1:
    '[data-testid="section"]:nth-child(4) .MuiV5-Autocomplete-inputRoot input',
  platformDropDownMenu2:
    '[data-testid="section"]:nth-child(4) [placeholder*="Search or select Operating System"]',
  platformListBox:
    '[data-testid="entityfield-value"] , div.MuiV5-Autocomplete-popper [role="listbox"]',
  ...
  deleteDialog:
    '.MuiPaper-root.MuiDialog-paper.MuiDialog-paperScrollPaper.MuiDialog-paperWidthSm.MuiPaper-elevation24.MuiPaper-rounded[role="dialog"]',
...
export const DISCONNECT_USER_SENTENCE =
  "Users that don’t match Always-On rules are allowed to disconnect from the network";
...
export function deleteRowByIndex(rowIndex) {
  ...
  cy.get("@btn").click();
  cy.get(LOCATORS.deleteDialog).contains("button", "Delete").click();
}

export function assertSavedSuccessfully() {
  // Save and assert pop-up message
  cy.get(LOCATORS.saveButton).click();
  cy.get(LOCATORS.popUpmessage).should("be.visible").contains(SAVE_MESSAGE); //constant
}
