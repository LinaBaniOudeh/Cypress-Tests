...
describe("Ip Allocation Page", () => {
  ...
  it("Test dynamic ip input field false assertion", () => {
    ipAllocationHelper.typeInvalidInput(
      ipAllocationHelper.LOCATORS.dynamicIpInputField,
      ipAllocationHelper.INVALID_IPS[0],
      ipAllocationHelper.MSG.invalidCIDRFormat
    );
    ipAllocationHelper.assertDisabledSaveButton(
      ipAllocationHelper.LOCATORS.saveButton
    );
  });

  it("check validation on adding a static ip without adding a user rule", () => {
    ipAllocationHelper.typeValidInput(
      ipAllocationHelper.LOCATORS.staticIpInputField,
      ipAllocationHelper.VALID_IPS_Range[0]
    );
    ipAllocationHelper.setRadioButtonState(
      ipAllocationHelper.LOCATORS.staticIpsToggleButton,
      true
    );
  ...
  it("check validation on adding a user static ip that not in the range ", () => {
    ...
    ipAllocationHelper.addSDPUser(
      ipAllocationHelper.USERS[0].userName,
      ipAllocationHelper.INVALID_IPS[1]
    );
  ...
  it("check validation on adding a user static ip in the range ", () => {
    ...
    ipAllocationHelper.addSDPUser(
      ipAllocationHelper.USERS[0].userName,
      ipAllocationHelper.USERS[0].userIP
    );
    ...
    ipAllocationHelper.clearInputField(
      ipAllocationHelper.LOCATORS.staticIpInputField
    );
    ipAllocationHelper.assertSave(ipAllocationHelper.MSG.savedSuccessfully);
  });
  ...
  it("check validation on adding a duplicate sdp user with exsisted static ip", () => {
    ...
    ipAllocationHelper.assertSave(ipAllocationHelper.MSG.savedSuccessfully);
    ipAllocationHelper.addSDPUser(
      ipAllocationHelper.USERS[1].userName,
      ipAllocationHelper.USERS[0].userIP
    );
  ...
  it("Test overlaps ip in dynamic and static ip range", () => {
    ipAllocationHelper.typeValidInput(
      ipAllocationHelper.LOCATORS.dynamicIpInputField,
      ipAllocationHelper.VALID_IPS_Range[1]
    );
    ipAllocationHelper.setRadioButtonState(
      ipAllocationHelper.LOCATORS.staticIpsToggleButton,
      true
